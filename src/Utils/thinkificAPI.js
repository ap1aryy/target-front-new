...
export const getCourseDetails = async (userId, courseId) => {
  ...
  export const generateInvoice = async (userId, appId, count, setInvoiceGenerated, type) => {
...
export const getAllChapters = async (app_id, user_id) => {
  try {
    const response = await axios.post(`${BASE_URL}/chapters`, {user_id : user_id, app_id: app_id});
    return response.data;
  } catch (error) {
