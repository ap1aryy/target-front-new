        ...
        async () => {
          ...
          try {
            ...
            if (i18n.language === "ru") {
              alert(
                `${t(
                  course_data.id.toString() + ".Course_name"
            ...
            navigate(`/courses/${course_data.id}`, {
    ...
    <div className={`popup-menu`} onClick={(e) => e.stopPropagation()}>
      <List>
        <Section style={{ marginTop: 8 }}>
          <Cell
            subhead={t("Course_name")}
            style={{ pointerEvents: "none", border: "none", marginBottom: 5 }}
            children={`${t(course_data.id.toString() + ".Course_name")}`}
          />

          {stage > 1 && selectedPlan ? (
            <Cell
              style={{ borderColor: "hsl(0deg 0% 0% / 0%)" }}
            ...
            <Section style={{ marginTop: 8 }}>
              {options.map((option) => (
                <Cell
                  multiline
                  key={option.id}
                  subtitle={option.benefits
                    .map((benefit) => t(benefit))
                    .join(", ")}
